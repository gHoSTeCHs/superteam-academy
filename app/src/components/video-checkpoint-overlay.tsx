...
import { Trophy, SkipForward, AlertCircle, CheckCircle2, XCircle } from 'lucide-react';
...
export function VideoCheckpointOverlay({
  ...
  const mcqConfig = checkpoint.questionData?.responseConfig as McqConfig | undefined;
              ...
              {options && (
                <div className="space-y-2">
                  {options.map((option) => {
                    ...
                    const showCorrect = isSelected && answerStatus === 'correct';
                    const showIncorrect = isSelected && answerStatus === 'incorrect';

                    return (
                      <button
                        key={option.label}
                        type="button"
                        disabled={answerStatus === 'correct' || exhaustedAttempts}
                        onClick={() => handleOptionSelect(option.label, option.is_correct)}
                        className={cn(
                          ...
                          showIncorrect && 'border-red-500 bg-red-500/10 text-red-400',
                      ...
                      >
                        ...
                        {showCorrect && <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-green-400" />}
                        {showIncorrect && <XCircle className="h-4 w-4 flex-shrink-0 text-red-400" />}
                      </button>
                    );
                  })}
                </div>

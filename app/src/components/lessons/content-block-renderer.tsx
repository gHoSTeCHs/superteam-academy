...
const calloutConfig = {
  info: { icon: InfoIcon, bg: 'bg-blue-500/10', border: 'border-blue-500/30', iconColor: 'text-blue-500' },
  warning: { icon: AlertTriangleIcon, bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', iconColor: 'text-yellow-500' },
  tip: { icon: LightbulbIcon, bg: 'bg-primary/10', border: 'border-primary/30', iconColor: 'text-primary' },
};

export function getPaneTarget(block: ContentBlock): PaneTarget {
  if (block.data.type === 'code_example' || block.data.type === 'code_challenge') {
    return 'right';
  }
  return 'left';
}
  ...
  switch (data.type) {
      ...
      return (
        <div className="space-y-2">
          ...
          <CodeEditor
            ...
            readOnly
            height="280px"
            className={cn('rounded-lg border border-border', data.filename && 'rounded-t-none')}
          />
        </div>
      );
          ...
          {data.responseConfig &&
            ...
            Array.isArray(data.responseConfig.options) && (
              <ul className="mt-3 space-y-2">
                {(data.responseConfig.options as Array<{ label: string; text: string }>).map(
                  (opt) => (
                    <li key={opt.label}>
    ...
    case 'callout': {
      ...
      return (
        ...
        >
          <CalloutIcon className={cn('mt-0.5 size-5 shrink-0', config.iconColor)} />

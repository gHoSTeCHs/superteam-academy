    ...
    <div className={depth > 0 ? "ml-5 border-l border-border/40 pl-4" : ""}>
      <div
        className={
          ...
          (isLeaf ? " hover:bg-bg-raised" : "")
        }
      >
        {hasChildren ? (
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="mt-0.5 cursor-pointer border-none bg-transparent p-0 text-[10px] text-muted-foreground transition-transform duration-150"
        ...
        <div className="flex-1">
            ...
            <div className="mt-2 space-y-1">
              {q.matchingPairs.map((pair, i) => (
                <div
                  ...
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  <span className="w-[140px] shrink-0 rounded border border-border bg-bg-raised px-2 py-1 text-center font-medium">
                    {pair.left}
                  </span>
                  <span className="text-primary">{"\u2194"}</span>
                  <span className="flex-1 rounded border border-[var(--opt-correct-border)] bg-[var(--opt-correct-bg)] px-2 py-1">
                    {pair.right}
                  </span>
                </div>
              ))}
              {q.matchingDistractors && q.matchingDistractors.length > 0 && (
                <div className="mt-1 flex flex-wrap gap-1">
                  {q.matchingDistractors.map((d, i) => (
                    <span
                      key={i}
                      className="rounded border border-border bg-bg-raised px-2 py-0.5 text-[10px] text-muted-foreground line-through"
                    >
                      {d}
                    </span>
                  ))}
          ...
          {q.orderItems && (
            <div className="mt-2 space-y-1">
              {q.orderItems.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 rounded border border-border px-3 py-1.5 text-[12px]"
                  style={{ fontFamily: "var(--font-body)" }}
                >
          ...
          {q.type === "calculation" && q.calculationAnswer && (
            <div className="mt-2 flex items-center gap-2">
              <span
                className="rounded border border-[var(--opt-correct-border)] bg-[var(--opt-correct-bg)] px-3 py-1 text-[12px] font-bold"
                style={{ fontFamily: "var(--font-body)" }}
              >
                = {q.calculationAnswer}
                {q.calculationUnit ? ` ${q.calculationUnit}` : ""}
              </span>
            </div>
          )}
          {q.gapOptions && (
                    ...
                    {gap.options.map((opt, oi) => (
                      <span
                        ...
                        className={
                          "rounded px-2 py-0.5" +
                          (oi === gap.correct
                            ? " border border-[var(--opt-correct-border)] bg-[var(--opt-correct-bg)] font-bold"
                            : " bg-bg-raised")
                        }
                      >
                        {opt}
                ...
                >
                  <thead>
                    <tr className="bg-bg-raised">
                      ...
                      {q.matrixRight.map((_, i) => (
                        <th
                          key={i}
                          className="border-b border-border px-2 py-1.5 text-center font-bold text-primary"
                        >
                  ...
                  <tbody>
                    {q.matrixLeft.map((item, li) => (
                      ...
                      >
                        <td className="border-r border-border px-2 py-1.5 font-medium">
                          {li + 1}. {item}
                        </td>
                        {q.matrixRight!.map((_, ri) => (
                          <td key={ri} className="px-2 py-1.5 text-center">
                            {q.matrixMapping![li]?.includes(ri) ? (
                              <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-[var(--opt-correct-dot)] text-[9px] text-white">
                                {"\u2713"}
                              </span>
                            ) : (
          ...
          {q.choiceGroup && (
            <div className="mt-2 rounded border border-accent/30 bg-accent/5 px-3 py-1.5">
              <span
                className="text-[10px] font-bold text-accent-foreground"
                style={{ fontFamily: "var(--font-body)" }}
              >
                Answer {q.choiceGroup.required.join(", ")} (required) + choose{" "}
                {q.choiceGroup.chooseN} from {q.choiceGroup.optional.join(", ")}
              </span>
            </div>
          )}
        </div>

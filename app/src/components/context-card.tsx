  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="mb-2 flex items-center gap-2">
        ...
        <Badge variant="neutral" className="ml-auto text-[9px]">id: {context.id}</Badge>
      </div>
      {context.title && (
      ...
      {context.content && (
        <div className="rounded-md bg-bg-raised p-3">
          <p className="text-[12px] leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>{context.content}</p>
        </div>
      )}
      ...
      {context.wordBank && (
        <div className="flex flex-wrap gap-1.5">
          {context.wordBank.map((w) => (
            <span key={w} className="rounded border border-border bg-bg-raised px-2 py-0.5 text-[11px]">{w}</span>
          ))}
        </div>
      )}
    </div>

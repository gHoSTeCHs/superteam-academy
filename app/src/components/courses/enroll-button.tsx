...
export function EnrollButton({
  ...
  if (status === 'completed') {
    return (
      <div className={cn('space-y-3', className)}>
        ...
        <Button variant="outline" size="md" onClick={onViewCredential} className="w-full">
          View Credential
        </Button>
      </div>
    );
  ...
  if (status === 'enrolling') {
    return (
      <Button variant="primary" size="md" disabled className={cn('w-full', className)}>
        <Loader2Icon className="size-4 animate-spin" />
        <span style={{ fontFamily: 'var(--font-body)' }}>Signing transaction...</span>
      </Button>
    );
  }
